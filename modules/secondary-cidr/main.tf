...
locals {
  create_public_subnets = local.num_public_subnets > 0
}
...
resource "aws_subnet" "public" {
  ...
  tags = merge(
    {
      Name = try(
        local.public_subnet_names[count.index],
        format("${var.name}-${var.public_subnet_suffix}-%s", element(data.aws_availability_zones.available.names, count.index % length(data.aws_availability_zones.available.names)))
      )
    },
    var.tags
  )
...
locals {
  num_public_route_tables = var.create_multiple_public_route_tables ? local.num_public_subnets : 1
}
...
resource "aws_route_table_association" "public" {
  count = local.create_public_subnets ? local.num_public_subnets : 0
...
resource "aws_subnet" "private" {
  ...
  tags = merge(
    {
      Name = try(
        local.private_subnet_names[count.index],
        format("${var.name}-${var.private_subnet_suffix}-%s", element(data.aws_availability_zones.available.names, count.index % length(data.aws_availability_zones.available.names)))
      )
    },
    var.tags
  )
...
resource "aws_route_table" "private" {
  count = local.create_private_subnets && local.num_private_subnets > 0 ? local.nat_gateway_count : 0
...
# ################################################################################
# # Associate Internet Gateway -- need to confirm
# ################################################################################

# resource "aws_route" "private_ipv6_egress" {
#   count =  var.create_egress_only_igw && var.enable_ipv6 && local.num_private_subnets > 0 ? local.nat_gateway_count : 0
...
locals {
  nat_gateway_count = var.single_nat_gateway ? 1 : var.one_nat_gateway_per_az ? length(local.azs) : local.num_private_subnets
